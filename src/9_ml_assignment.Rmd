...
The best iteration of my RMSE for tyring to achieve an improved value are:

train-rmse: 0.525835
val-rmse: 0.676373

This is only very slightly improved from the above model. 


### What hypertuning parameters most impacted your model performance?

I mostly moved the eta and gamma parameters of the model. 
...
prepData = function(df) {
  ...
  df_prep %>% 
    mutate(NR= med_Nir_corr/med_Red_corr,
           BG= med_Blue_corr/med_Green_corr,
           BR= med_Blue_corr/med_Red_corr, 
           NDWI = (med_Green_corr - med_Nir_corr) / (med_Green_corr + med_Nir_corr),
           NDTI = (med_Red_corr -med_Green_corr)/(med_Red_corr+med_Green_corr),
           NDVI = (med_Nir_corr - med_Red_corr)/(med_Nir_corr + med_Red_corr))
}
...
band_met5_feats_U <- c('med_Blue_corr', 'med_Green_corr', 'med_Red_corr', 'med_Nir_corr', 'NR', 'BG', 'BR', 'tot_sol_rad_KJpm2_5', 'max_temp_degK_5', 'min_temp_degK_5', 'tot_precip_m_5', 'mean_wind_mps_5', "NDWI", "NDVI", "NDTI")
...
```{r}
##No, adding the three spectral indices that I chose did not improve the results of my dataset. The R-squared value decreased and the RMSE for the trained and validate models were farther apart. 
```
